...
def _process() -> None:
    ...
    AOA = get_AOA()
    control_limiter = clamp(40000*pow(IAS,-2)*(0.9+0.1*Fuel)*(0.5 if VerticalG<1 else 1),0.01,1)
    Glimiter = get_Glimiter(-3,9)
    ...
    canard_pitch = get_canard_pitch(pitch_control) + AOA_feed - (AOA/45) + get_AOA_damper() + autotrim
...
def get_AOA_feed(max_feedback) -> float:
    global AOA
    target_AOA = (30+30*clamp01(-Activate8)-10*clamp01(abs(smooth(Roll,2))*2))*smooth(clamp(Pitch,-1,0),1)
    result = clamp((target_AOA-AOA)*0.02,0,max_feedback)
    return result*inverselerp(30,80,IAS)

def get_AOA_damper() -> float:
    global AOA
    return -rate(AOA)*0.01*inverselerp(200,100,IAS)*clamp01(-Activate8)
